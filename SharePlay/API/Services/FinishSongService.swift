...
class FinishSongService {
    
    init(){}
    func requestService(partyId: String, song: Song, requestCallback: RequestCallback) -> Void {
        let request: Request = Request.init()
        request.setUrl(url: "finishSong/")
